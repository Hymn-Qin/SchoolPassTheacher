	...
	public View getView(int arg0, View arg1, ViewGroup arg2) {
		...
		if (item.imageList != null && item.imageList.size() > 0) {
			...
			if (sourcePath!=null) {
				...
				PubMehods.loadBitmapUtilsPic(utilsXiao,holder.iv,"file://"+sourcePath);
				//cache.displayBmp(holder.iv, thumbPath, sourcePath, callback);
			}
			
		} else {
			Bitmap bimap = BitmapFactory.decodeResource(act.getResources(),
					R.drawable.side_lost_add);
			holder.iv.setImageBitmap(bimap);

...
import com.yuanding.schoolteacher.service.Api.AppNotice_ReceResult_AppNoticeUnReadOrRead_CallBack;
...
/**
 * 通知接收结果页面（身边-》通知列表--》通知详情--》图文通知接收结果
 * @author Administrator
 *
 */
public class B_Side_Notice_Main_Detail_0_ReceiveResult_AppNotice2 extends A_0_CpkBaseTitle_Navi{
	 ...
	 int mCurrentPageType; // 1: 未读 2 已读
...
//	 boolean isAllowInvative = false;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mCurrentPageType = getIntent().getExtras().getInt("acy_type");;
		messageId = getIntent().getExtras().getString("messageId");
		
		if(1==mCurrentPageType){
			setTitleText("未读");
		}else if(2==mCurrentPageType){
			setTitleText("已读");
		}
	...
	private void readCache() {
		...
		jsonObject = maACache.getAsJSONObject(AppStrStatic.cache_key_appnotice_unread_or_read + mCurrentPageType + messageId + A_0_App.USER_UNIQID);
	...
	private void showInfo(JSONObject jsonObject) {
		...
		try {
			...
			setTopTextContent(mAppNoticeUnReadOrRead);
...
//			        }
	...
	private void startReadData(String keyword) {
        A_0_App.getApi().getNoticeReceiverResult_AppNoticeUnReadOrRead(A_0_App.USER_TOKEN, mCurrentPageType,messageId, keyword, 1, new AppNotice_ReceResult_AppNoticeUnReadOrRead_CallBack() {
			
			@Override
			public void onSuccess(Cpk_NoticeReceiverResult_AppNotice_UnReadOrRead mAppNoticeUnReadOrRead) {
                if (isFinishing())
                    return;
                havaSuccessLoadData = true; 
				if(null!=mAppNoticeUnReadOrRead){
					/**
					 * 判断列表是否有数据
					 */
					if(null!=mAppNoticeUnReadOrRead.getUserlist() && mAppNoticeUnReadOrRead.getUserlist().size()>0){
						
						list_lack = mAppNoticeUnReadOrRead.getUserlist();
						mread_or_unreadAdapter.notifyDataSetChanged();
...
//							if(1==mAppNoticeUnReadOrRead.getAllow_invite()){
...
//              A_0_App.getInstance().CancelProgreDialog(B_Side_Notice_Main_Detail_0_ReceiveResult_AppNotice2.this);
                PubMehods.showToastStr(B_Side_Notice_Main_Detail_0_ReceiveResult_AppNotice2.this, msg);
                if(null!=mgv_noticereceiver){
                    mgv_noticereceiver.onRefreshComplete();
                }
	...
	private void getMoreData(int page_no,String keyword) {
        A_0_App.getApi().getNoticeReceiverResult_AppNoticeUnReadOrRead(A_0_App.USER_TOKEN, mCurrentPageType,messageId, "", page_no, new AppNotice_ReceResult_AppNoticeUnReadOrRead_CallBack() {
			
			@Override
			public void onSuccess(Cpk_NoticeReceiverResult_AppNotice_UnReadOrRead mAppNoticeUnRead) {
                if (isFinishing())
                    return;
				if(null!=mAppNoticeUnRead){
					...
					if(null!=mAppNoticeUnRead.getUserlist() && mAppNoticeUnRead.getUserlist().size()>0){
						...
						PubMehods.showToastStr(B_Side_Notice_Main_Detail_0_ReceiveResult_AppNotice2.this,"没有更多了");
					}
...
//              A_0_App.getInstance().CancelProgreDialog(B_Side_Notice_Main_Detail_0_ReceiveResult_AppNotice2.this);
	 ...
	 * 设置顶部文字内容
	 */
	private void setTopTextContent(Cpk_NoticeReceiverResult_AppNotice_UnReadOrRead mAppNoticeUnReadOrRead){
		if(1==mCurrentPageType){
			int unread_num = mAppNoticeUnReadOrRead.getUnread_num();
			int unactive_num = mAppNoticeUnReadOrRead.getUnactive_num();
			Integer unread_numInt = unread_num;
			Integer unactive_numInt = unactive_num;
									
			//更改tx_detailrule的部分文本颜色
			String text_front = "当前 ";
			String text_middle = " 人APP信息未读   有 ";
			String text_last = " 人未安装APP";
			SpannableStringBuilder builder = new SpannableStringBuilder(text_front+unread_num+text_middle+unactive_num+text_last);  	
			
			//ForegroundColorSpan 为文字前景色，BackgroundColorSpan为文字背景色    
			ForegroundColorSpan greenSpan2 = new ForegroundColorSpan(getResources().getColor(R.color.blue1));  
			builder.setSpan(greenSpan2, text_front.length(), text_front.length()+unread_numInt.toString().length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE); 
			
			ForegroundColorSpan greenSpan3 = new ForegroundColorSpan(getResources().getColor(R.color.red2));  
			builder.setSpan(greenSpan3, text_front.length()+unread_numInt.toString().length()+text_middle.length(), text_front.length()+unread_numInt.toString().length()+text_middle.length()+unactive_numInt.toString().length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE); 
			
			tx_read_or_unread_num.setText(builder); 
		}else if(2==mCurrentPageType){
			int read_num = mAppNoticeUnReadOrRead.getRead_num();
			Integer read_numInt = read_num;
			
			String text_front = "当前 ";
			String text_last = " 人已读";
			//更改tx_detailrule的部分文本颜色
			SpannableStringBuilder builder = new SpannableStringBuilder(text_front+read_num+text_last);  		  
			//ForegroundColorSpan 为文字前景色，BackgroundColorSpan为文字背景色    
			ForegroundColorSpan greenSpan2 = new ForegroundColorSpan(getResources().getColor(R.color.main_color));  
			builder.setSpan(greenSpan2, text_front.length(), text_front.length()+read_numInt.toString().length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE); 
			tx_read_or_unread_num.setText(builder); 
		}
	...
	public class MyGridView_NoticeReceiverResult_Aapter extends BaseAdapter {
		...
		public View getView(final int posi, View converView, ViewGroup arg2) {
			...
			if (converView == null) {
				...
				converView = LayoutInflater.from(B_Side_Notice_Main_Detail_0_ReceiveResult_AppNotice2.this).inflate(R.layout.item_notice_receiverresult_userhead, null);
			...
			if(null!=list_lack && list_lack.size()>0 && null!=holder){
				...
				if(1==mCurrentPageType){
					...
					if(2!=list_lack.get(posi).getStatus()){															
						...
						//更改tx_detailrule的部分文本颜色
						SpannableStringBuilder builder = new SpannableStringBuilder("未读");  		  
						//ForegroundColorSpan 为文字前景色，BackgroundColorSpan为文字背景色    
						ForegroundColorSpan greenSpan2 = new ForegroundColorSpan(getResources().getColor(R.color.blue1));  
						builder.setSpan(greenSpan2, 0, 2, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE); 
						holder.tv_recestatus.setText(builder); 
    ...
    private void sendInviteApp() {
        A_0_App.getApi().send_AppNoticeInviteInstall_Req(messageId, A_0_App.USER_TOKEN, new AppNotice_InviteInstallAppCallBack() {
            ...
            public void onSuccess(String message) {
                if (isFinishing())
                    return;
                PubMehods.showToastStr(B_Side_Notice_Main_Detail_0_ReceiveResult_AppNotice2.this, message);  
//                isAllowInvative = false;
            }
        },new Inter_Call_Back() {
            ...
            switch (connectionStatus) {
                ...
                case KICKED_OFFLINE_BY_OTHER_CLIENT:// 用户账户在其他设备登录，本机会被踢掉线
                    ...
                    class LooperThread extends Thread {
                        public void run() {
                            Looper.prepare();
                            A_0_App.getInstance().showExitDialog(B_Side_Notice_Main_Detail_0_ReceiveResult_AppNotice2.this, AppStrStatic.kicked_offline());
                            Looper.loop();
                        }
                    }

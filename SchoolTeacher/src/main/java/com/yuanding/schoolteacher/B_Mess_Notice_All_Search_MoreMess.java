...
import com.yuanding.schoolteacher.service.Api.InterMessSearchList;
		...
		public View getView(final int posi, View converView, ViewGroup arg2) {
			...
			converView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (mLecturesList.get(posi).getRes_user_id().equals(mLecturesList.get(posi).getUser_id())) {
						Intent intent = new Intent(B_Mess_Notice_All_Search_MoreMess.this,
						...
						intent.putExtra("acy_detail_id", mLecturesList.get(posi)
								.getMessage_id());
						intent.putExtra("type", mLecturesList.get(posi).getType());
						intent.putExtra("isHaveSend", 1+""); //是否是已发送页面
						startActivity(intent);
					}else{
						if (mLecturesList.get(posi).getType().equals("4")) {// 短信文本消息
							Intent intent = new Intent(B_Mess_Notice_All_Search_MoreMess.this,B_Mess_Notice_Detail_MessText_Teacher.class);
							intent.putExtra("acy_type", 2);// 正常列表进入
							...
							Intent intent = new Intent(B_Mess_Notice_All_Search_MoreMess.this,B_Mess_Notice_Detail_Teacher.class);
							intent.putExtra("acy_type", 2);// 正常列表进入
							intent.putExtra("message_id", mLecturesList.get(posi).getMessage_id());
							intent.putExtra("mess_content",mLecturesList.get(posi).getTitle());
							startActivityForResult(intent, 1);

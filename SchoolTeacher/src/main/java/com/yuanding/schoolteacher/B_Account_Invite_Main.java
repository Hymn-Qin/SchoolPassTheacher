...
import com.yuanding.schoolteacher.B_Account_Invite_InviteRecord.InviteRecordAdapter;
...
import com.yuanding.schoolteacher.service.Api.BindInviterCallBack;
import com.yuanding.schoolteacher.service.Api.Inter_Call_Back;
import com.yuanding.schoolteacher.service.Api.InviteInfoCallBack;
...
public class B_Account_Invite_Main extends A_0_CpkBaseTitle_Navi implements OnClickListener{
	...
	Button btn_share;
	TextView tx_top_rec_info_top,tx_detailrule,tv_temp_invite; //顶部和底部的文字内容
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		isInstallWeiXin = ShareUtil.isInstallTargetMedia(B_Account_Invite_Main.this, SHARE_MEDIA.WEIXIN);
		initView();		
		initConfig();
		readCache();
...
//				showLoadResult(false,true, false);
			...
			/** 获取邀请链接和邀请规则链接  */
			shareInviteUrl = jsonObject.optString("rec_url");
			inviteRuleUrl = jsonObject.optString("rec_info_url");	
...
//				showLoadResult(false,true, false);
	 ...
	 * 绑定我的推荐人信息
	 */
	private void bindInviter(String phone) {
		...
		A_0_App.getApi().reqBindInviter(A_0_App.USER_TOKEN, phone, new BindInviterCallBack() {
            ...
            public void onFailure(String msg) {
                if(isFinishing())
                    return;
                PubMehods.showToastStr(mContext, msg);
            }
		...
		public void onClick(View v) {
			...
			case R.id.btn_committjrphonenum:
				if(!edit_tjrphonenum.getText().toString().isEmpty()){
					//hidekeyboard();
					bindInviter(edit_tjrphonenum.getText().toString());
				}else{
			...
			case R.id.btn_share:
				...
				ShareDialog.Builder builder = new Builder(mContext, "微信好友","微信朋友圈","复制链接",isInstallWeiXin, new DialogBtnClickCallBack() {
...
//            logD("上传融云数据完毕");
	...
	private void shareReq(int sharetype) {
		...
		ShareUtil.sendShareReqToWeiXin(default_ShareTitle,default_ShareContent, shareInviteUrl,"",mSHARE_MEDIA, B_Account_Invite_Main.this, new UMShareListener() {

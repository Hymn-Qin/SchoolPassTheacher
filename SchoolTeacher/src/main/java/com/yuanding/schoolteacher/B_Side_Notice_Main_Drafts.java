 ...
 * 草稿箱页面
 */
public class B_Side_Notice_Main_Drafts extends Fragment {
...
//	private DisplayImageOptions options;
	...
	public B_Side_Notice_Main_Drafts() {
		super();
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		viewone = inflater.inflate(R.layout.notice_sent_main, container,
				false);
      ...
      mPullDownView.setOnScrollListener(new OnScrollListener() {
		...
		mPullDownView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int posi,
				...
				click_posi = posi - 1;
				Intent intent = new Intent(getActivity(),B_Side_Notice_Main_Sent_SMS.class);
				intent.putExtra("acy_type", 2);// 正常列表进入
				intent.putExtra("notice_type", AppStrStatic.cache_key_notice_draft+A_0_App.USER_UNIQID);
				intent.putExtra("acy_detail_id", mCourseList.get(posi - 1).getMessage_id());
				intent.putExtra("content", mCourseList.get(posi - 1).getDesc());
	...
	private void readCache() {
		...
		jsonObject = maACache.getAsJSONObject(AppStrStatic.cache_key_notice_draft+A_0_App.USER_UNIQID);
	...
	private void startReadData(int page_no, final boolean pullRefresh
		...
		A_0_App.getApi().getSideNoticeList(type,getActivity(), A_0_App.USER_TOKEN,
				String.valueOf(page_no), new InterSideNoticeList() {
		...
		public View getView(int posi, View converView, ViewGroup arg2) {
			...
			if (converView == null) {
				...
				converView = LayoutInflater.from(getActivity()).inflate(
						R.layout.item_side_notice_sent, null);
				...
				holder.tv_sent_sign = (TextView) converView
						.findViewById(R.id.tv_sent_sign);
				holder.tv_sent_content = (TextView) converView
						.findViewById(R.id.tv_sent_content);
			...
			if (img_url != null && img_url.length() > 0 && !img_url.equals("")) {
				...
				/*imageLoader.displayImage(img_url, holder.iv_acy_lsit_img,
						options);*/
				PubMehods.loadBitmapUtilsPic(bitmapUtils, holder.iv_acy_lsit_img, img_url);
			} else {
			...
			if(A_0_App.isShowAnimation==true){
			 if(posi>A_0_App.draft_curPosi)
			 {
				A_0_App.draft_curPosi=posi;
	...
	class ViewHolder {
		...
		TextView tv_sent_sign,tv_sent_content;
		View view_bottom;
	}
...
//            logD("上传融云数据完毕");
	...
	public void onResume() {
		super.onResume();
		if (A_0_App.SIDE_NOTICE==0||A_0_App.SIDE_NOTICE==1||A_0_App.SIDE_NOTICE==2) {
			clearBusinessList(false);
			have_read_page = 1;
			startReadData(have_read_page, false);
		} 

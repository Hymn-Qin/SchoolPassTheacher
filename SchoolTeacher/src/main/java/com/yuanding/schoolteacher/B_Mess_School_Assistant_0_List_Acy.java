...
import com.yuanding.schoolteacher.service.Api.InterNoticeOfficalDelete;
import com.yuanding.schoolteacher.service.Api.InterSchoolAssistantList;
...
public class B_Mess_School_Assistant_0_List_Acy extends A_0_CpkBaseTitle_Navi {
    ...
    private String type,title_name,acy_list_type,img_course ="";//消息类型 1:考勤 2：审核 3：通知  4：课程表 5:短信
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getExtras() != null) {
            acy_type = getIntent().getExtras().getInt("acy_type",2);
            type = getIntent().getExtras().getString("type");//类型
            if (acy_type == 1) {
                // 推送进入 message_id = PubMehods.getSharePreferStr(this, "mCurrentClickNotificationMsgId");
                title_name = A_0_App.getInstance().getTitle_name();
                acy_list_type = type;
            }else if (acy_type == 2){
                //列表进入
    ...
    private void judge_read(List<Cpk_School_Assistant_List> list,boolean firsetEnter) {
        if(!acy_list_type.equals("2") || !acy_list_type.equals("5")){//审核、短信不做全部未读处理
                ...
                public void onClick(View arg0) {
                    ...
                    if(mCourseList.get(posi).getType().equals("1")){
                        ...
                        if(mCourseList.get(posi).getJump_module().equals("SIDE")){
                            Intent intent = new Intent(B_Mess_School_Assistant_0_List_Acy.this,B_Side_Notice_Main_Detail_0.class);
                            ...
                            if (mCourseList.get(posi).getMessage_type().equals("1")) {
                                intent.setClass(B_Mess_School_Assistant_0_List_Acy.this, B_Mess_Notice_Detail_Teacher.class);
                            } else if (mCourseList.get(posi).getMessage_type().equals("4")) {            
                                intent.setClass(B_Mess_School_Assistant_0_List_Acy.this, B_Mess_Notice_Detail_MessText_Teacher.class);                   
                            }else{
                                intent.setClass(B_Mess_School_Assistant_0_List_Acy.this, B_Mess_Notice_Detail_Sys_Teacher.class);
                            }
                            intent.putExtra("school_notice", mCourseList.get(posi).getMessage_id());
                            intent.putExtra("acy_type", 2);
                    ...
                    }else if(mCourseList.get(posi).getType().equals("5")) {//短信充值明细

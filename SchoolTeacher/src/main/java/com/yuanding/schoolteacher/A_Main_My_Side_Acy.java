...
import com.yuanding.schoolteacher.R.color;
import com.yuanding.schoolteacher.base.A_0_CpkBaseTitle;
import com.yuanding.schoolteacher.bean.Cpk_Banner_Mode;
import com.yuanding.schoolteacher.bean.Cpk_Module_List;
import com.yuanding.schoolteacher.bean.Cpk_Top_Info;
import com.yuanding.schoolteacher.bean.Cpk_Top_List;
import com.yuanding.schoolteacher.service.Api.InterGetInfo;
...
import com.yuanding.schoolteacher.view.banner.ADInfo;
import com.yuanding.schoolteacher.view.banner.CycleViewPager;
import com.yuanding.schoolteacher.view.banner.CycleViewPager.ImageCycleViewListener;
import com.yuanding.schoolteacher.view.banner.ViewFactory;
...
public class A_Main_My_Side_Acy extends A_0_CpkBaseTitle {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		bitmapUtils = A_0_App.getBitmapUtils(A_Main_My_Side_Acy.this,
				R.drawable.icon_side_nothing, R.drawable.icon_side_nothing,true);
		...
		rela_side_info.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				intent.setClass(A_Main_My_Side_Acy.this,
						B_Side_Info_0_Main_Acy.class);
	...
	private void readCache() {//&& !A_0_App.getInstance().getNetWorkManager().isNetWorkConnected() 暂时去掉会引起加载变白
		...
		jsonObject = maACache.getAsJSONObject(AppStrStatic.cache_key_mySide
				+ A_0_App.USER_UNIQID);
	...
	private void showInfo(JSONObject jsonObject) {
			...
			if (state == 1) {
				...
				JSONArray jsonArrayItem2 = jsonObject
						.optJSONArray("moduleList");
				JSONObject jsonArrayItem3 = jsonObject.optJSONObject("info");
				mlistContact = JSON.parseArray(jsonArrayItem + "",
						Cpk_Banner_Mode.class);
				moduleContact = JSON.parseArray(jsonArrayItem2 + "",
						Cpk_Module_List.class);
				top_List = JSON.parseObject(jsonArrayItem3 + "",
						Cpk_Top_List.class);
				...
				cpk_Top_Infos = JSON.parseArray(cpk_Top_List.getTop_list(),
						Cpk_Top_Info.class);
	...
	//enter_acy_type,首页快捷方式进入为1，身边通知进入为2
    private void startAcyNotice(Context packageContext, Class<?> cls) {
        Intent intent = new Intent();
        intent.setClass(packageContext, cls);
	...
	protected void showDialog() {
		final GeneralDialog upDateDialog = new GeneralDialog(
				A_Main_My_Side_Acy.this, R.style.Theme_GeneralDialog);
		...
		upDateDialog.showLeftButton(R.string.pub_temporarily_open,
				new OnClickListener() {
					...
					public void onClick(View v) {
					    startAcyNotice(A_Main_My_Side_Acy.this,
								B_Side_Notice_Main.class);
						A_0_App.getInstance().saveNoticeGesture("2");
						upDateDialog.cancel();
						PubMehods.showToastStr(A_Main_My_Side_Acy.this,
								R.string.str_open_gesture_set);
					}
				});
		upDateDialog.showRightButton(R.string.pub_immediately_open,
				new OnClickListener() {
					@Override
					public void onClick(View v) {
						Intent intent = new Intent(A_Main_My_Side_Acy.this,
								B_Side_Notice_GestureSet_Setting.class);
						intent.putExtra("enter_acy", 1);
						startActivity(intent);
						upDateDialog.cancel();
					}
				});
...
//				    if (A_0_App.USER_STATUS.equals(AppStrStatic.USER_ROLE_HAVA_CERTIFIED)) {
    					intent.setClass(A_Main_My_Side_Acy.this,B_Side_Acy_list_Detail_Acy_Teacher.class);
...
//                    }
				...
				case 2:// 讲座
				    if (A_0_App.USER_STATUS.equals(AppStrStatic.USER_ROLE_HAVA_CERTIFIED)) {
    					intent.setClass(A_Main_My_Side_Acy.this,B_Side_Lectures_Detail_Acy_Teacher.class);
    					intent.putExtra("acy_type", 2);
	...
	public void startListtenerRongYun() {
		RongIM.getInstance().setConnectionStatusListener(
		...
		public void onChanged(ConnectionStatus connectionStatus) {
			...
			case CONNECTED:// 连接成功。
				A_Main_My_Message_Acy
						.logD("A_Main_My_Side_Acy模板监听=教师——connectRoogIm()，融云连接成功");
				break;
			case DISCONNECTED:// 断开连接。
				A_Main_My_Message_Acy
						.logD("A_Main_My_Side_Acy模板监听=教师——connectRoogIm()，融云连接~~~~~~~~~~~~~~~~~~~~~~~~~~~~~断开连接");
				// A_0_App.getInstance().showExitDialog(A_Main_My_Side_Acy.this,getResources().getString(R.string.token_timeout));
				break;
			case CONNECTING:// 连接中。
				A_Main_My_Message_Acy
						.logD("A_Main_My_Side_Acy模板监听=教师——connectRoogIm()，融云连接中");
				break;
			case NETWORK_UNAVAILABLE:// 网络不可用。
				A_Main_My_Message_Acy
						.logD("A_Main_My_Side_Acy模板监听=教师——connectRoogIm()，融云连接网络不可用");
				break;
			case KICKED_OFFLINE_BY_OTHER_CLIENT:// 用户账户在其他设备登录，本机会被踢掉线
				A_Main_My_Message_Acy
						.logD("A_Main_My_Side_Acy模板监听=教师——connectRoogIm()，用户账户在其他设备登录，本机会被踢掉线");
				class LooperThread extends Thread {
					public void run() {
						Looper.prepare();
						A_0_App.getInstance().showExitDialog(
								A_Main_My_Side_Acy.this,
		...
		public View getView(final int posi, View converView, ViewGroup arg2) {
			...
			if (lmap.get(posi) == null) {
				...
				holder.iv_side_img = (ImageView) converView
						.findViewById(R.id.iv_side_image);
			...
			if (NumColumns == 4) {
				holder.tv_side_name.setTextSize(15);
			} else {
				holder.tv_side_name.setTextSize(12);
			}

			if (map_red.size() > 0
					&& map_red.get(module_Lists.get(posi).getCode()) != null) {
				if (map_red.get(module_Lists.get(posi).getCode()).equals("1")) {
					holder.tv_side_info_read_count.setVisibility(View.VISIBLE);
				} else {
					holder.tv_side_info_read_count
			...
			holder.tv_side_info_read_count
					.setBackgroundResource(R.drawable.icon_message_noread);
			RelativeLayout.LayoutParams lp_menpiao2 = new RelativeLayout.LayoutParams(
			...
			lp_menpiao2.width = width;
			lp_menpiao2.height = width;
			lp_menpiao2.setMargins(left, top, 0, 0);// 4个参数按顺序分别是左上右下
			holder.tv_side_info_read_count.setLayoutParams(lp_menpiao2);
			String uri = module_Lists.get(posi).getIcon_url();
		    if (null != uri && !"".equals(uri) && !"null".equals(uri)) {
			...
			converView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					if (module_Lists.get(posi).getApp_type().equals("1")) {
						if (map_red.get(module_Lists.get(posi).getCode()) != null&& map_red
										.get(module_Lists.get(posi).getCode()).equals("1")) {
						...
						if (module_Lists.get(posi).getCode().equals("MESSAGE")) {
							// 通知
							...
							if (A_0_App.getInstance().mNoticeGesure.equals("0")) {// 未设置过给提示
								...
								Intent intent_1 = new Intent(A_Main_My_Side_Acy.this,B_Side_Notice_Gesture_Verifcation.class);
								intent_1.putExtra("enter_acy", 1);
								startActivity(intent_1);
							} else if (A_0_App.getInstance().mNoticeGesure.equals("2")) {// 为关
							    startAcyNotice(A_Main_My_Side_Acy.this,B_Side_Notice_Main.class);
							}

						} else if (module_Lists.get(posi).getCode().equals("SALARY")) {
							//工资
						    if (A_0_App.USER_STATUS.equals(AppStrStatic.USER_ROLE_HAVA_CERTIFIED)) {
                                startActivity(new Intent(A_Main_My_Side_Acy.this,B_Side_Wages_Acy.class));
                            }else if (A_0_App.USER_STATUS.equals(AppStrStatic.USER_ROLE_NO_SUPPLEMENTARY_INFO)){
                                A_0_App.getInstance().enter_Perfect_information(A_Main_My_Side_Acy.this,true);
                            }else if (A_0_App.USER_STATUS.equals(AppStrStatic.USER_ROLE_AUDIT_FAILURE)){
                                A_0_App.getInstance().enter_Perfect_information(A_Main_My_Side_Acy.this,true);
							...
							// 失物招领
							// startActivity(new Intent(A_Main_My_Side_Acy.this,B_Side_Lost_Found_Acy.class));空白页
						    if (A_0_App.USER_STATUS.equals(AppStrStatic.USER_ROLE_HAVA_CERTIFIED)) {
							...
							// 考勤
						    if (A_0_App.USER_STATUS.equals(AppStrStatic.USER_ROLE_HAVA_CERTIFIED)) {
                                startActivity(new Intent(A_Main_My_Side_Acy.this,B_Side_Attence_Main.class));
                            }else if (A_0_App.USER_STATUS.equals(AppStrStatic.USER_ROLE_NO_SUPPLEMENTARY_INFO)){
                                A_0_App.getInstance().enter_Perfect_information(A_Main_My_Side_Acy.this,true);
                            }else if (A_0_App.USER_STATUS.equals(AppStrStatic.USER_ROLE_AUDIT_FAILURE)){
                                A_0_App.getInstance().enter_Perfect_information(A_Main_My_Side_Acy.this,true);
							...
							// 活动
							startAcy(A_Main_My_Side_Acy.this,B_Side_Acy_list_Scy.class);
                        ...
                        } else if(module_Lists.get(posi).getCode().equals("ENROLL")){
							//迎新管理
							if (A_0_App.USER_STATUS.equals(AppStrStatic.USER_ROLE_HAVA_CERTIFIED)) {
								if (!PubMehods.isFastClick(AppStrStatic.WORD_COMMENT_MIN_LIMIT)) {
//									Intent courseIntent = new Intent(A_Main_My_Side_Acy.this, SDK_WebApp.class);
//									startActivity(courseIntent);
//									A_0_App.getInstance().showProgreDialog(A_Main_My_Side_Acy.this, "", false);

									Intent courseIntent = new Intent(A_Main_My_Side_Acy.this, SDK_WebView.class);
									courseIntent.putExtra("acy_type",1);
									startActivity(courseIntent);
									A_0_App.getInstance().showProgreDialog(A_Main_My_Side_Acy.this, "", false);
						...
						if (map_red.get(module_Lists.get(posi).getCode()) != null
								&& map_red.get(module_Lists.get(posi).getCode()).equals("1")) {
							map_red.remove(module_Lists.get(posi).getCode());
							map_red.put(module_Lists.get(posi).getCode(), "0");
							holder.tv_side_info_read_count.setVisibility(View.INVISIBLE);
							ToSayServiceInfo(A_0_App.USER_TOKEN, module_Lists.get(posi).getCode());
...
//			params.put("type", "1");// 1是教师，2是学生
//
//			String sign = PubMehods.getMD5("app_version" + app_version
//					+ "command" + "MODULEUPDATE" + "module" + type + "token"
//					+ token + "type" + "1" + AppStrStatic.ZCODE
//					+ AppStrStatic.ZCODE_VALUE);
//			params.put(AppStrStatic.SIGN, sign);
//
//			String msg = params.toString();
//			boolean isSend = A_Main_My_Message_Acy.iBackService
//					.sendMessage(msg);
//			Log.e("BackService", isSend ? "发送成功" : "发送失败");
//			if (!isSend) {
//				boolean isSend_Once = A_Main_My_Message_Acy.iBackService
//						.sendMessage(msg);
//				Log.e("BackService", isSend_Once ? "再次发送成功" : "再次发送失败");
//			} else {
//				A_0_App.getInstance().saveOAuth(map_red);
//			}
//			// PubMehods.showToastStr(A_Main_My_Message_Acy.this, isSend ?
//			// "发送成功" : "发送失败");
//		} catch (Exception e) {
...
//		}
    ...
    public static void logD(String msg) {
        com.yuanding.schoolteacher.utils.LogUtils.logD("A_Main_My_Side_Acy","A_Main_My_Side_Acy==>" + msg);
    }

    public static void logE(String msg) {
        com.yuanding.schoolteacher.utils.LogUtils.logE("A_Main_My_Side_Acy","A_Main_My_Side_Acy==>" + msg);
    }
}

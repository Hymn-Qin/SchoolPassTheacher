...
import com.yuanding.schoolteacher.service.Api.InterDelegatedAttdenceList;
 ...
 * 委派考勤主界面
 */
public class B_Side_Attence_Main_A10 extends Fragment{
    ...
    public B_Side_Attence_Main_A10() {
		super();
	}
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        iv_blank_title.setText("还没有委派考勤~");
		...
		sp = getActivity().getSharedPreferences("wxb", Context.MODE_PRIVATE);
      ...
      //**************************新增到这**********************
		...
		add_click.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View arg0) {
                startActivity(new Intent(getActivity(), B_Side_Attence_Main_Set_Contact.class));
            }
        });
...
//            showLoadResult(false, false, false, true);
        ...
        if (sp.getString(A_0_App.USER_UNIQID + "first", "").equals("1")) {
        	B_Side_Attence_Main.getInstance().updateChildTips(1, false);
		}else {
			B_Side_Attence_Main.getInstance().updateChildTips(1, true);
		}
        return viewone;
	}
    ...
    private void readCache( ) {
        ...
        jsonObject = maACache.getAsJSONObject(AppStrStatic.cache_key_side_delegatedattdence_list+A_0_App.USER_UNIQID);
	...
	private void getAttdenceList(final int page_no) {
	    A_0_App.getApi().getDelegatedAttdenceList(getActivity(), A_0_App.USER_TOKEN, String.valueOf(page_no), new InterDelegatedAttdenceList() {
            @Override
...
//	
	...
	public void onResume() {
        if (!firstLoad_Tag&&A_0_App.SIDE_ATTENCE==1) {
            have_read_page = 1;
            getAttdenceList(have_read_page);
        } else {
            logD(" 第一次进入页面不刷新");

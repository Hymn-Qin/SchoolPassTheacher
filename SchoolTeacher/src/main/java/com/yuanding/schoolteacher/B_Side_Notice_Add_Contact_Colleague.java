...
import com.yuanding.schoolteacher.service.Api.InterNoticeAddContact_Colleague;
...
/**
 * @author Jiaohaili 
 * @version 创建时间：2016年2月22日 上午10:56:19 添加通知收信人同事
 */
public class B_Side_Notice_Add_Contact_Colleague extends Fragment {
	...
	public static int colleague=1;//判断最终退出
	...
	public B_Side_Notice_Add_Contact_Colleague() {
		super();
	}
	...
	private void getAttdenceList(String is_back, final String organ_id) {
		A_0_App.getApi().NoticeAddContact_Colleague(getActivity(),
				A_0_App.USER_TOKEN, is_back, organ_id,
				new InterNoticeAddContact_Colleague() {
					...
					public void onSuccess(
						...
						if (back.equals("1")) {//返回
							if (A_0_App.colleague_temp_Contacts.size()==add_Contacts.size()) {//全选上一级被选中处理
								if (add_Contacts!=null) {
									add_Contacts.clear();
								}
	                             add_Contacts=mList;
	                             ...
	                             for (int i = 0; i < add_Contacts.size(); i++) {
									if (add_Contacts.get(i).getOrgan_id().equals(parentid)) {
										...
										A_0_App.colleague_temp_Contacts.add(add_Contacts.get(i));
									}
								}
	                             
	                             for (int i = 0; i <  A_0_App.colleague_result_Contacts.size(); i++) {
	 								for (int j = 0; j < add_Contacts.size(); j++) {
	 									if ( A_0_App.colleague_result_Contacts.size()>0) {
	 										if ( A_0_App.colleague_result_Contacts.get(i).getOrgan_id().equals(add_Contacts.get(j).getOrgan_id())) {
		 										 A_0_App.colleague_result_Contacts.remove(i);
		 										A_0_App.colleague_temp_Contacts.add(add_Contacts.get(j));
								 ...
								 A_0_App.colleague_result_Contacts.addAll(A_0_App.colleague_temp_Contacts);
								 if (add_Contacts!=null) {
										add_Contacts.clear();
									}
	                            add_Contacts=mList;
						...
						if (back.equals("0")) {//正常
                            ...
                            add_Contacts=mList;
                            if (A_0_App.colleague_all_Contacts.size()==0) {//保存所有不重复元素
                            	A_0_App.colleague_all_Contacts.addAll(mList);
                            	for (int i = 0; i < A_0_App.colleague_all_Contacts.size(); i++) {
                            		String temp=A_0_App.colleague_all_Contacts.get(i).getOrgan_id();
                            		A_0_App.colleague_all_Contacts.get(i).setNext(temp+",");
								}
							}else{
								...
								A_0_App.colleague_all_Contacts.addAll(mList);
								removeDuplicate(A_0_App.colleague_all_Contacts);
								String temp="";
                             for (int i = 0; i <A_0_App.colleague_all_Contacts.size(); i++) {
                            	 
                            	 if (A_0_App.colleague_all_Contacts.get(i).getNext().split(",").length>1) {
										
                            		 if (A_0_App.colleague_all_Contacts.get(i).getOrgan_id().equals(organ_id)) {
                            			 temp=A_0_App.colleague_all_Contacts.get(i).getNext();
 									}
									if (A_0_App.colleague_all_Contacts.get(i).getParentid().equals(organ_id)) {
										String m=A_0_App.colleague_all_Contacts.get(i).getNext();
										A_0_App.colleague_all_Contacts.get(i).setNext(temp+m);
									}	
							...
							if (temp.equals("1")) {//全部选中，重复的移除
								...
								A_0_App.colleague_temp_Contacts.addAll(add_Contacts);
							}else{//有的选中重复的移除
								
								if (A_0_App.colleague_result_Contacts.size()>0) {
									Iterator<Cpk_Side_Attence_Add_Contact> iter = A_0_App.colleague_result_Contacts.iterator(); 
									while(iter.hasNext()){  
				...
				public void onClick(View arg0) {//选中及相关处理
					if (add_Contacts.get(position).getOrgan_name().equals("全部")) {
						if (add_Contacts.get(position).isCheck() == false) {
							...
							Iterator<Cpk_Side_Attence_Add_Contact> iter = A_0_App.colleague_temp_Contacts.iterator(); 
							while(iter.hasNext()){  
								Cpk_Side_Attence_Add_Contact s = iter.next(); 
							 ...
							 A_0_App.colleague_temp_Contacts.addAll(add_Contacts);
							 Iterator<Cpk_Side_Attence_Add_Contact> iter2= A_0_App.colleague_temp_Contacts.iterator(); 
								while(iter2.hasNext()){  
					...
					if (add_Contacts.get(position).isCheck() == false) {
						...
						A_0_App.colleague_temp_Contacts.add(add_Contacts.get(position));
						
						for (int j = 0; j < A_0_App.colleague_result_Contacts.size(); j++) {
							for (int i = 0; i < A_0_App.colleague_all_Contacts.size(); i++) {
								if (A_0_App.colleague_result_Contacts.get(j).getOrgan_id().equals(A_0_App.colleague_all_Contacts.get(i).getOrgan_id())) {
									
									A_0_App.colleague_result_Contacts.get(j).setNext(A_0_App.colleague_all_Contacts.get(i).getNext());
							}
						}
					}
						...
						for (int i = 0; i < A_0_App.colleague_temp_Contacts.size(); i++) {
							if (A_0_App.colleague_temp_Contacts.get(i).getOrgan_id().equals(add_Contacts.get(position).getOrgan_id())) {
								A_0_App.colleague_temp_Contacts.remove(i);
							}
						}
 ...
 */
	public class FinishReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context arg0, Intent arg1) {
			if (arg1.getIntExtra("position", 0)==1) {

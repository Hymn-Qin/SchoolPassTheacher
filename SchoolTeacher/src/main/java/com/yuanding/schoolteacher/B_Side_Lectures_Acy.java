...
import com.yuanding.schoolteacher.service.Api.InterLectureList;
...
public class B_Side_Lectures_Acy extends A_0_CpkBaseTitle_Navi{
      ...
      //**************************新增到这**********************
		...
		mPullDownView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int posi,long arg3) {
				...
				Intent intent = new Intent(B_Side_Lectures_Acy.this,B_Side_Lectures_Detail_Acy_Teacher.class);
	...
	private void showInfo(JSONObject jsonObject) {
		...
		try {
			...
			mServierTime=jsonObject.getLong("time");
			if(null!=mPullDownView){
			    mPullDownView.onRefreshComplete();
	...
	private void getMoreLecture(int page_no) {
		...
		A_0_App.getApi().getLectureList(B_Side_Lectures_Acy.this, A_0_App.USER_TOKEN,String.valueOf(page_no), new InterLectureList() {
			@Override
			public void onSuccess(List<Cpk_Side_Lectures_List> mList,long servitime) {
				if (isFinishing())
					return;
	...
	public class Mydapter extends BaseAdapter{
		...
		public View getView(int posi, View converView, ViewGroup arg2) {
			...
			if (converView == null) {
				...
				holder.tv_lecture_status=(TextView) converView.findViewById(R.id.tv_lecture_status);//讲座进行状态
				holder.tv_lecture_time=(TextView) converView.findViewById(R.id.tv_join_end_time);
				holder.tv_time_title=(TextView) converView.findViewById(R.id.tv_time_title);
				holder.tv_lecture_detial_time=(TextView) converView.findViewById(R.id.tv_join_end_minute);
				holder.ll_lecture_time=(LinearLayout) converView.findViewById(R.id.ll_lecture_time);
				holder.tv_join_end_hour=(TextView) converView.findViewById(R.id.tv_join_end_hour);
			...
			if(server_time<start_time)
			{
			    ...
			    holder.tv_join_end_hour.setText(PubMehods.getRemainTime(time_to_start));
			    holder.tv_lecture_detial_time.setText(PubMehods.getRemainMinute(time_to_start));
			...
			}else if(server_time<end_time)
			{
				...
				holder.tv_join_end_hour.setText(PubMehods.getRemainTime(time_to_end));
				holder.tv_lecture_detial_time.setText(PubMehods.getRemainMinute(time_to_end));
			}else
			{
				//已结束
	...
	class ViewHolder{
		...
		TextView tv_lecture_detial_time;
		LinearLayout ll_lecture_time;
		TextView tv_join_end_hour;
	}

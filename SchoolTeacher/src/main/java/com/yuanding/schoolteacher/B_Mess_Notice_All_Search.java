	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		listview_mess.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (mess_Notices.get(position).getRes_user_id().equals(mess_Notices.get(position).getUser_id())) {
					Intent intent = new Intent(B_Mess_Notice_All_Search.this,
					...
					intent.putExtra("acy_detail_id", mess_Notices.get(position)
							.getMessage_id());
					intent.putExtra("type", mess_Notices.get(position).getType());
					intent.putExtra("isHaveSend", 1+""); //是否是已发送页面
					
					startActivity(intent);
				}else{
					if (mess_Notices.get(position).getType().equals("4")) {// 短信文本消息
						Intent intent = new Intent(B_Mess_Notice_All_Search.this,B_Mess_Notice_Detail_MessText_Teacher.class);
						intent.putExtra("acy_type", 2);// 正常列表进入
						...
						Intent intent = new Intent(B_Mess_Notice_All_Search.this,B_Mess_Notice_Detail_Teacher.class);
						intent.putExtra("acy_type", 2);// 正常列表进入
						intent.putExtra("message_id", mess_Notices.get(position).getMessage_id());
						intent.putExtra("mess_content",mess_Notices.get(position).getTitle());
						startActivityForResult(intent, 1);

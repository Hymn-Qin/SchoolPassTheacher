...
import com.yuanding.schoolteacher.service.Api.InteSideCourse;
...
//            R.drawable.bg_course_04};
    ...
    private int acy_type;//1:推送，11:正常进入，12:校务助手进入
...
//                    ACache.get(context).put(AppStrStatic.cache_key_b_side_course+A_0_App.USER_UNIQID+week,jsonObject);
    ...
    private void readData(String week,String message_id) {
        A_0_App.getApi().getSideCourse(B_Side_Course_Acy.this, A_0_App.USER_TOKEN,week,message_id,
                new InteSideCourse() {
                    @Override
        ...
        for (int i = 1; i <= DAY_TOTLE_NODE_NUM; i++) {// 12表示一天的课节数
            for (int j = 1; j <= 8; j++) {// 8表示8列
                ...
                if (j == 1) {
                    ...
                    tx.setCompoundDrawables(null, drawable, null, null);//画在右边

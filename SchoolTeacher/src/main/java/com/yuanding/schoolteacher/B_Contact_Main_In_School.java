...
import com.yuanding.schoolteacher.service.Api.InterSchoolInOutList;
...
/**
 * @author Jiaohaili 
 * @version 创建时间：2016年1月5日 下午4:25:39 
 * 校内
 */
public class B_Contact_Main_In_School extends Fragment {
    ...
    private void readCache() {
        ...
        jsonObject = maACache.getAsJSONObject(AppStrStatic.cache_key_school_in
                + A_0_App.USER_UNIQID);
	...
	private void showInfo(JSONObject jsonObject) {
		// TODO Auto-generated method stub
		List<Cpk_School_In_And_Out> mList = getList(jsonObject);
		...
		if (mList != null && mList.size() > 0) {
			...
			showLoadResult(true,false, false, true);
		}
		demo_swiperefreshlayout.setRefreshing(false);  
				...
				if (parent != null && parent.length() > 0) {
					for (int j = 0; j < parent.length(); j++) {
						...
						school_in.setParent_id(parent
								.getJSONObject(j).optString(
										"parent_id"));
						JSONArray child = parent.optJSONObject(
								j).optJSONArray("info");
		...
		public View getView(final int position, View converView, ViewGroup arg2) {
			...
			converView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					namePhone = cate_list.get(position).getInfo_name();
					telephone = cate_list.get(position).getPhone();
					A_0_App.getInstance().callSb(getActivity(), namePhone, telephone, new A_0_App.PhoneCallBack() {
								@Override
								public void sPermission() {
									MPermissions.requestPermissions(B_Contact_Main_In_School.this, REQUECT_CODE_CALLPHONE, new String[]{
...
//	);
	...
	{
		PubMehods.callPhone(getContext(), telephone);
	}

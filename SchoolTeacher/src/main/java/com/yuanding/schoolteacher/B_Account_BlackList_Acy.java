...
public class B_Account_BlackList_Acy extends A_0_CpkBaseTitle_Navi {
	...
	RongIM.getInstance().getBlacklist(new GetBlacklistCallback() {
				@Override
				public void onSuccess(String[] arg0) {//此处为Uniqid32位
	                if (isFinishing())
	                    return;
					try {
...
//							setTextBtn("管理");
	...
	public class MyAdapter extends BaseAdapter {
		...
		public View getView(int position, View converView, ViewGroup arg2) {
		    ...
		    if (null != list.get(position).getPhoto_url() && !"".equals(list.get(position).getPhoto_url()) && !"null".equals(list.get(position).getPhoto_url())) {

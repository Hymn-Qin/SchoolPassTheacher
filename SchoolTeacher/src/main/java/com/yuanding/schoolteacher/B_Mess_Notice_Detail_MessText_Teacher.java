 ...
 * 短信文本通知详情
 */
public class B_Mess_Notice_Detail_MessText_Teacher extends A_0_CpkBaseTitle_Navi {
  ...
  //  protected BitmapUtils bitmapUtils,bitmapUtilsBanner;
    ...
    private boolean readCommentData =false,havaSuccessLoadData = false;
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (s.length() > 0 && !"".equals(s.toString()))
                {
                    ...
                    if (AppStrStatic.WORD_COMMENT_MAX_LIMIT < s.length()) {
                        PubMehods.showToastStr(B_Mess_Notice_Detail_MessText_Teacher.this, "已达到最大字数限制");
				...
				if(A_0_App.USER_STATUS.equals("2")){
    				if (str != null && !(str.equals("")) && str.length() >= AppStrStatic.WORD_COMMENT_MIN_LIMIT) {
                        if(!judgeNoNullStr(str)){
                            PubMehods.showToastStr(B_Mess_Notice_Detail_MessText_Teacher.this,"请输入3个字以上的评论内容");
                            return;
                        }
    					if (!PubMehods.isFastClick(AppStrStatic.INTERVAL_RONGYUN_CONNECT)) {
    						commentAcy(message_id, log_id, str);
    					} else {
    						PubMehods.showToastStr(B_Mess_Notice_Detail_MessText_Teacher.this,
    								"您的评论过于频繁！");
    					}
                    ...
                    PubMehods.showToastStr(B_Mess_Notice_Detail_MessText_Teacher.this,"您还未审核通过，暂不支持评论");
                }
    ...
    private void readCache(String message_id) {
		...
		jsonObject = maACache.getAsJSONObject(AppStrStatic.cache_key_notice_detail_text+A_0_App.USER_UNIQID+message_id);
    ...
    private void loadData(Cpk_Notice_Detail notice,int temp){
        ...
        if (notice.getContent().equals("")||notice.getContent()==null) {
            ...
            PubMehods.showToastStr(B_Mess_Notice_Detail_MessText_Teacher.this, "信息不存在！");
        }else{
            if(A_0_App.USER_STATUS.equals("2")){
...
//                 {
    ...
    private void addHeadView(PullToRefreshListView listview) {
        ...
        View view1 = LayoutInflater.from(B_Mess_Notice_Detail_MessText_Teacher.this).inflate(R.layout.activity_mess_notice_head_text, null);
        ...
        tv_mess_detail_from=(TextView) view1.findViewById(R.id.tv_message_detail_notice_from);
...
//    webSettings.setDefaultFontSize(16);
       ...
       listView2.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View arg0, MotionEvent arg1) {
				...
				tv_notice_detail_comment_text.setHint("评论一条");
				 PubMehods.closeKeybord(tv_notice_detail_comment_text, B_Mess_Notice_Detail_MessText_Teacher.this);
				return false;
			}
		});
    }
    ...
    private void readData(String message_id) {
        A_0_App.getApi().getNoticeDetail(B_Mess_Notice_Detail_MessText_Teacher.this, school_notice, A_0_App.USER_TOKEN, 
                message_id,AppStrStatic.cache_key_notice_detail_text+A_0_App.USER_UNIQID,is_share,new InterNoticeDetail() {
            @Override
            ...
            public void onFailure(String msg) {
                if (isFinishing())
                    return;
                if (jsonObject == null) {
                    PubMehods.showToastStr(B_Mess_Notice_Detail_MessText_Teacher.this, msg);
                    showLoadResult(false, false, true);
                }
                if (lv_notice_detail_comment != null) {
                     lv_notice_detail_comment.onRefreshComplete();
	...
	private void commentAcy(final String message_id,String log_id,String content) {
		A_0_App.getApi().add_Notice_Detail_Comment(message_id, log_id, A_0_App.USER_TOKEN, content, new InterAdd_Notice_Comment() {
			...
			public void onSuccess() {
				if(isFinishing())
					return;
				//A_0_App.getInstance().CancelProgreDialog(B_Mess_Notice_Detail_MessText_Teacher.this);
				
					PubMehods.showToastStr(B_Mess_Notice_Detail_MessText_Teacher.this, "评论成功");
	...
	private void getMoreComment(String message_id,int page_no) {
		...
		A_0_App.getApi().getNoticeCommentList(B_Mess_Notice_Detail_MessText_Teacher.this,A_0_App.USER_TOKEN,message_id,String.valueOf(page_no),new InterNoticeCommentList(){
            ...
            public void onSuccess(List<Cpk_Comment_detail> mList) {
                ...
                if (mList != null && mList.size() > 0) {
                    ...
                    PubMehods.showToastStr(B_Mess_Notice_Detail_MessText_Teacher.this, "没有更多了");
                }
                if (lv_notice_detail_comment!=null) {
					lv_notice_detail_comment.onRefreshComplete();
				}
    ...
    public class Mydapter extends BaseAdapter {
        ...
        public View getView(final int posi, View converView, ViewGroup arg2) {
			...
			if (converView == null) {
				converView = LayoutInflater.from(B_Mess_Notice_Detail_MessText_Teacher.this).inflate(R.layout.item_info_comment_more, null);
			...
			for (int i = 0; i < comment_List_All.size(); i++) {
				View view = LayoutInflater.from(B_Mess_Notice_Detail_MessText_Teacher.this).inflate(R.layout.item_side_notice_detail_more, null);
...
//                    statusPopup.dismiss();
//                }
//                Intent intent=new Intent(B_Mess_Notice_Detail_MessText.this, B_Mess_Forward_Select_Change.class);
...
//    
    ...
    protected void handleTitleBarEvent(int resId,View v) {
        ...
        case ZUI_RIGHT_BUTTON:
            if(A_0_App.USER_STATUS.equals("2")){
            	  Intent intent=new Intent(B_Mess_Notice_Detail_MessText_Teacher.this, B_Mess_Forward_Select.class);
                  intent.putExtra("title", detail_Notice.getTitle());
    ...
    private void goAcy() {
        if (acy_type == 3) {// 推送
            if (A_Main_Acy.getInstance() != null) {
                ...
                startAcy(B_Mess_Notice_Detail_MessText_Teacher.this, A_Main_Acy.class);
            }
        } else {// 正常进入
            goData();
            finish();
        ...
        public void onChanged(ConnectionStatus connectionStatus) {
            ...
            switch (connectionStatus) {
                ...
                case DISCONNECTED:// 断开连接。
                    ...
                    //A_0_App.getInstance().showExitDialog(B_Mess_Notice_Detail_MessText_Teacher.this,getResources().getString(R.string.token_timeout));
                    break;
                case CONNECTING:// 连接中。
                ...
                case KICKED_OFFLINE_BY_OTHER_CLIENT:// 用户账户在其他设备登录，本机会被踢掉线
                    ...
                    class LooperThread extends Thread {
                        public void run() {
                            Looper.prepare();
                            A_0_App.getInstance().showExitDialog(B_Mess_Notice_Detail_MessText_Teacher.this, AppStrStatic.kicked_offline());
                            Looper.loop();
                        }
                    }

...
import com.yuanding.schoolteacher.service.Api.InterNoticeList;
...
public class B_Mess_Notice_List extends A_0_CpkBaseTitle_Navi {
	...
	private String level;// 学校、院系之分
	private int click_posi = 0;
	private ACache maACache;
	private JSONObject jsonObject;

	private int LONGDELETE = 0;// 布局判断
    ...
    private boolean firsetEnterLoader = false,havaSuccessLoadData=false;
	...
	private void showInfo(JSONObject jsonObject) {
		...
		havaSuccessLoadData = true;
			...
			if (mlistContact != null && mlistContact.size() > 0) {
				judge_read(mlistContact,firsetEnterLoader);
					...
					public void onSuccess(List<Cpk_Notice_List> mList,int unreceipt_num) {
						...
						if(!onlyUpdateCache){//更新缓存用
	                        ...
	                        if (mList != null && mList.size() > 0) {
	                            ...
	                            adapter.notifyDataSetChanged();
	                            if (pullRefresh)
	                                PubMehods.showToastStr(B_Mess_Notice_List.this,
	                                        "刷新成功");
	                        } else {
	                        	 showTitleBt(ZUI_RIGHT_TEXT, false);
	                                judge_read(mList,firsetEnterLoader);
...
//								B_Mess_Notice_List.this);
						if (mList != null && mList.size() > 0) {
							...
							adapter.notifyDataSetChanged();
//							mPullDownView.getRefreshableView().setSelection(
//									totleSize + 1);
							judge_read(mCourseList,firsetEnterLoader);
						} else {
							PubMehods.showToastStr(B_Mess_Notice_List.this,
...
//			if (converView == null) {
//				converView = LayoutInflater.from(B_Mess_Notice_List.this)
...
//					holder.iv_notice_delete.setBackgroundDrawable(getResources()
...
//                //imageLoader.displayImage(image_url, holder.iv_notice_deatil_img, options);
//                if(holder.iv_notice_deatil_img.getTag() == null){
//                    bitmapUtils.display(holder.iv_notice_deatil_img, image_url);
...
//			 }
//			}
//			holder.liner_notice.setOnClickListener(new OnClickListener() {
//
//				@Override
...
//			});
//			holder.ll_notice
//					.setBackgroundResource(R.drawable.notice_list_shape);
...
//				    click_posi = posi;
////					holder.ll_notice
////							.setBackgroundResource(R.drawable.notice_list_press_shape);
//					
//					if (mCourseList.get(posi).getIs_default().equals("0")) {
//						if (mCourseList.get(posi).getType().equals("4")) {// 短信文本消息
//							Intent intent = new Intent(B_Mess_Notice_List.this,
//									B_Mess_Notice_Detail_MessText_Teacher.class);
//							intent.putExtra("acy_type", 2);// 正常列表进入
//							intent.putExtra("message_id", mCourseList.get(posi)
//									.getMessage_id());
//							intent.putExtra("mess_content",
//									mCourseList.get(posi).getDesc());
//							intent.putExtra("sign",  mCourseList.get(posi).getApp_msg_sign());
//							startActivityForResult(intent, 1);
//						} else {// 正常消息
//							Intent intent = new Intent(B_Mess_Notice_List.this,
//									B_Mess_Notice_Detail_Teacher.class);
//							intent.putExtra("acy_type", 2);// 正常列表进入
//							intent.putExtra("message_id", mCourseList.get(posi)
//									.getMessage_id());
//							intent.putExtra("mess_content",
...
//					} else {// 系统消息
//						Intent intent = new Intent(B_Mess_Notice_List.this,
//								B_Mess_Notice_Detail_Sys_Teacher.class);
//						intent.putExtra("acy_type", 2);
//						intent.putExtra("message_id", mCourseList.get(posi)
//								.getMessage_id());
//						intent.putExtra("log_id", mCourseList.get(posi)
//								.getLog_id());
//						intent.putExtra("sign",  mCourseList.get(posi).getApp_msg_sign());
//						startActivityForResult(intent, 1);
//					}
...
//							holder2.btn_waitsurenotice.setText(mCurrentWaitSureNoticeNumber+" 条待确认信息");
				    ...
				    if(null==converView){
				    	defaultNoticeMsgView = LayoutInflater.from(B_Mess_Notice_List.this).inflate(R.layout.item_notificity_list_havesurereceiverbtn, arg2, false);
					    holder = new ViewHolder();
...
//								holder.iv_notice_deatil_img, options);
					...
					if (mCourseList.get(mPosi).getMl_status().equals("1")) {
						...
						if (mCourseList.get(mPosi).getType().equals("4")) { //短信 
							...
							Log.i("aaa", "getView: " + "aaaaa"+ mCourseList.get(mPosi).getTitle()+ mCourseList.get(mPosi).getIs_appendix());
							switch (mCourseList.get(mPosi).getIs_appendix()){
								case 0:
									holder.tv_notice_detail_title.setText(""
											+ mCourseList.get(mPosi).getTitle());
						...
						if (mCourseList.get(mPosi).getType().equals("4")) {
							...
							switch (mCourseList.get(mPosi).getIs_appendix()){
								case 0:
									holder.tv_notice_detail_title.setText(mCourseList.get(mPosi)
											.getTitle());
									holder.tv_notice_detail_title.setCompoundDrawables(null, null, null, null);
									break;
								case 1:
						...
						public void onClick(View arg0) {
							...
							if (mCourseList.get(mPosi).getIs_default().equals("0")) {
								if (mCourseList.get(mPosi).getType().equals("4")) {// 短信文本消息
									Intent intent = new Intent(B_Mess_Notice_List.this,B_Mess_Notice_Detail_MessText_Teacher.class);
									intent.putExtra("acy_type", 2);// 正常列表进入
									intent.putExtra("message_id", mCourseList.get(mPosi).getMessage_id());
									intent.putExtra("mess_content", mCourseList.get(mPosi).getDesc());
									startActivityForResult(intent, 1);
								} else {// 正常消息
									Intent intent = new Intent(B_Mess_Notice_List.this,B_Mess_Notice_Detail_Teacher.class);
									intent.putExtra("acy_type", 2);// 正常列表进入
									intent.putExtra("message_id", mCourseList.get(mPosi).getMessage_id());
									intent.putExtra("mess_content",mCourseList.get(mPosi).getDesc());
									startActivityForResult(intent, 1);
								...
								Intent intent = new Intent(B_Mess_Notice_List.this,B_Mess_Notice_Detail_Sys_Teacher.class);
								intent.putExtra("acy_type", 2);
								intent.putExtra("message_id", mCourseList.get(mPosi).getMessage_id());

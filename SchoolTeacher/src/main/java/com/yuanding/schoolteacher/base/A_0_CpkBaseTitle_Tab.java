...
import com.yuanding.schoolteacher.view.frament.TitleIndicator;
...
public abstract class A_0_CpkBaseTitle_Tab extends FragmentActivity implements
   ...
   public void setZuiYouText(String resid){
	...
	public void onPageSelected(int position) {
		...
		A_0_App.SIDE_ATTENCE=position;
		
		
		
		
		Editor editor = sp.edit();
		editor.putString(A_0_App.USER_UNIQID + "first", "1");
		editor.commit();
		
		 if (sp.getString(A_0_App.USER_UNIQID + "first", "").equals("1")) {
	        	updateChildTips(1, false);
			}else {
				updateChildTips(1, true);
			}
        A_0_App.contace_tab_posi = mCurrentTab;
        A_Main_My_Contact_Acy.logD(mCurrentTab + "通讯录选项卡");
        if (position==0) {
     ...
     * @param showTips
     */
    public void updateChildTitle(int postion, String title) {
        mIndicator.updateChildTitle(postion, title);
    }

		...
		case ZUI_RIGHT_BUTTON: 
			...
			if (item_type.equals(A_3_3_Complete_marer_Acy.TITLE_USER_TURE_NAME)){
				...
				if(str != null && !str.equals("")&& str.length() > 0){
					if (str.length() >= 3 && str.length() <= 30) {
						if(PubMehods.isNumeric(str)){
							...
							PubMehods.showToastStr(A_3_3_Complete_Marer_Modify_Acy.this, "工号为3~30位的数字");
							return;
						}
					}else{
						PubMehods.showToastStr(A_3_3_Complete_Marer_Modify_Acy.this, "工号为5~30位的数字");
						return;
					}
				}else{
					PubMehods.showToastStr(A_3_3_Complete_Marer_Modify_Acy.this,"请输入你的工号");
					return;
				}
			}
				...
				if(str != null && !str.equals("")&& str.length() > 0){
					...
					PubMehods.showToastStr(A_3_3_Complete_Marer_Modify_Acy.this,"请输入邀请人手机号");
					return;
				}
			}
			this.finish();

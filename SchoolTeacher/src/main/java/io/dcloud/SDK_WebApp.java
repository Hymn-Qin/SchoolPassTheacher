		...
		if (hasFocus && Build.VERSION.SDK_INT >= 21) {
			try {
				//if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
					Window window = SDK_WebApp.this.getWindow();
					window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
					window.setStatusBarColor(SDK_WebApp.this.getResources().getColor(R.color.transparent));
...
//	ProgressDialog pd = null;
	...
	public void onCoreInitEnd(ICore coreHandler) {
		...
		String appBasePath = "/apps/enroll";

		// 设置启动参数,可在页面中通过plus.runtime.arguments;方法获取到传入的参数
		String args = A_0_App.USER_TOKEN;
